...
function Todos(props) {
	...
	const expiredTodos = formattedTodosData
		...
		.filter(
			(todo) => moment(todo.will_end_at).diff(currentDate, "days") < 0
		)
		.map((todo) => {
	...
	const tommorowTodos = formattedTodosData
		.filter((todo) => todo.will_end_at)
		.filter(
			(todo) => moment(todo.will_end_at).diff(currentDate, "days") === 1
		);
...
function mapDispatchToProps(dispatch) {
	return {
		...
		addNote: (note, token) => dispatch(addNote(note, token)),
		addTodo: (todoData, token) => dispatch(addTodo(todoData, token)),
		setBody: (todoData, token) => dispatch(setBody(todoData, token)),
		setNoteBody: (noteData, token) =>
			dispatch(setNoteBody(noteData, token)),
		deleteNote: (noteData, token) => dispatch(deleteNote(noteData, token)),
		completeTodo: (todoData, token) =>
			dispatch(completeTodo(todoData, token)),
		restoreTodo: (todoData, token) =>
			dispatch(restoreTodo(todoData, token)),
		deleteTodo: (todoData, token) => dispatch(deleteTodo(todoData, token)),
	};
}
