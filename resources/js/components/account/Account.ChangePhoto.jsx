...
export default function ChangePhoto({ canvasPropsObject, updatePhoto }) {
	...
	const changeCanvasZeroCoordinates = useCallback(() => {
		throttle(() => {
			setCanvasZeroCoordX(
				canvasElement.current.getBoundingClientRect().left
			);
			setCanvasZeroCoordY(
				canvasElement.current.getBoundingClientRect().top
			);
		}, 40);
	});
				...
				img.addEventListener("load", () => {
					...
					if (img.width > img.height) {
						...
						canvasUpdate({
							...
							src: src,
							sxRatio:
								(img.width * halfHeight) / width / halfHeight,
						...
						canvasUpdate({
							...
							src: src,
							sxRatio:
								(img.width * halfWidth) / width / halfWidth,
	...
	function changeStateObject(clientX, clientY) {
		...
		const mouseOffsetDeltaX =
			clientX - canvasZeroCoordX - mouseClickOffsetX;
		const mouseOffsetDeltaY =
			clientY - canvasZeroCoordY - mouseClickOffsetY;
		...
		<div className="upload-photo">
			<div {...getRootProps({ style })}>
				...
				{isDragActive ? (
					...
					<p>
						Перетащите фотографию сюда или нажмите, чтобы выбрать.
					</p>
				)}
			</div>
