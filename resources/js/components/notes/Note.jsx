...
export function Note({ note, avatar, setNoteBody, deleteNote, CSRF_TOKEN }) {
	...
	function changeNoteRequest() {
		setNoteBody({ id: note.id, body: noteInputText }, CSRF_TOKEN);
		setIsFocusedNote(false);
	}

	function deleteNoteRequest() {
		deleteNote({ id: note.id }, CSRF_TOKEN);
	}
