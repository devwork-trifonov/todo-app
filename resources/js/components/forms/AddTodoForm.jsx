...
export default function AddTodoForm({ addTodo, successAddedTodo }) {
  ...
  const inputFieldsReducer = (state, action) => {
    switch (action.type) {
      ...
      case "addDateField":
        if (
          state.inputFields[state.inputFields.length - 1].type === "text" &&
      ...
      case "valueInput":
        if (action.payload.indexInArray === state.lastInput.indexInArray) {
          return {
            ...state,
      ...
      case "delete":
        const arrayWithoutTargetField = state.inputFields
          ...
          .filter((field) => field.indexInArray !== action.payload.indexInArray)
          .map((field) => {
        ...
        for (let i = 1; i < arrayWithoutTargetField.length; i++) {
          if (
            ...
            mergedArray.push({
              type: "text",
              inputIndex: arrayWithoutTargetField[i - 1].inputIndex,
              indexInArray: i - 1,
              id: generateId(),
              value: textBuffer,
              top: 0,
  ...
  return (
    <div className="add-todo-form" ref={formRef}>
      ...
      <DatePickerModal
        ...
        className={
          formState & formStateMap.DATEPICKER ? "date-list_opened" : "date-list"
        }
      />
      ...
      >
        <div
          className={
            formState & formStateMap.HAS_TEXT ? "options" : "options_closed"
          }
        >
          <button
            onClick={() => {
